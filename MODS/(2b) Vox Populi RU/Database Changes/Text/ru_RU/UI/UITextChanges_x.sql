...
UPDATE Language_ru_RU
SET Text = 'Уровень безопасности отражает сложность шпионажа в городе. Чем выше это значение, тем больше времени потребуется иностранным шпионам для выполнения шпионских миссий. Чтобы повысить безопасность в городе, можно построить шпионские здания, такие как тюрьма и констебль.[NEWLINE][NEWLINE]Нажмите, чтобы отсортировать города по уровню безопасности.'
...
UPDATE Language_ru_RU
SET Text = 'Шпионские миссии нельзя проводить в [ICON_CITY_STATE] городах-государствах, поэтому у них нет уровня [ICON_CITY_SECURITY] безопасности.[NEWLINE][NEWLINE]Отправка [ICON_SPY] шпиона в город-государство повысит ваше [ICON_INFLUENCE] влияние там за счёт [ICON_PUPPET] подтасовки выборов. Если в городе находится больше одного шпиона, наибольший шанс подтасовать выборы имеет шпион, который дольше всех находится в этом городе-государстве.'
...
UPDATE Language_ru_RU
SET Text = 'Возможности для {1_SpyRank} {2_SpyName}:[NEWLINE][NEWLINE][ICON_BULLET] Переместиться в город-государство и попытаться [COLOR_POSITIVE_TEXT]подтасовать выборы[ENDCOLOR].[NEWLINE][ICON_BULLET] Переместиться в город, принадлежащий крупной цивилизации, и собирать разведданные для проведения [COLOR_POSITIVE_TEXT]шпионской миссии[ENDCOLOR].[NEWLINE][ICON_BULLET] Переместиться в столицу, принадлежащую крупной цивилизации, и [COLOR_POSITIVE_TEXT]налаживать связи[ENDCOLOR] как дипломат.[NEWLINE][ICON_BULLET] Переместиться в один из ваших городов, чтобы выбрать [COLOR_POSITIVE_TEXT]специализацию контршпиона[ENDCOLOR].'
